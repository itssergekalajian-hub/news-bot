...
# Caps how many posts a single run can send. Kept small on purpose: the
# workflow runs every 5 minutes (see .github/workflows/news-bot.yml), so
# posting just a few stories per frequent run makes the channel read like a
# natural, steadily-updating feed instead of dumping a big batch at once.
# Anything over the cap isn't lost - it stays cached as "relevant" and is
# picked up by the next run a few minutes later, so a real burst of breaking
# news still clears quickly, just spaced out rather than flooded. Keeping it
# small also keeps each run's summarization calls well under the LLM free
